...
export default function EditProject({ projectId, onClose, onUpdated }: EditTaskModalProps) {
  ...
  useEffect(() => {
    ...
    const fetchTask = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/getProjectById/${projectId}`, {
          headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(
        `${API_BASE_URL}/api/updateProject/${projectId}`,
        {
          ...form,
  ...
  return (
    <div className="fixed inset-0 z-5 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-lg w-full max-w-lg p-6 relative">
        ...
        <h2 className="text-2xl font-semibold text-center mb-4">Edit project</h2>
